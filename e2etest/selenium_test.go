...
func TestFrontendSeleniumE2E(t *testing.T) {
	...
	err = waitTillElementFound(wd, "[data-testid='mock-login-user-btn']", 10*time.Second)
	if err != nil {
	...
	err = waitTillElementFound(wd, "[data-testid='dashboard-header']", 10*time.Second)
	if err != nil {
	...
	err = waitTillElementFound(wd, "[data-testid='login-card']", 10*time.Second)
	if err != nil {
...
func waitTillElementFound(wd selenium.WebDriver, selector string, timeout time.Duration) error {
